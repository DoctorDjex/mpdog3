    ...
    if(path.extname(file) == ".mp3" || path.extname(file) == ".MP3")
    {
        ...
        id3({ file: 'incoming_songs/' + path.basename(file), type: id3.OPEN_LOCAL }, function(err, tags) {
            if(err){
                console.log(err);
                console.log(file);
                var theFile = 'incoming_songs/' + path.basename(file);
                var picStream = fs.createReadStream(theFile).pipe(fs.createWriteStream('incoming_songs/copy-' + path.basename(file)));
